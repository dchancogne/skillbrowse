...
Push a `vX.Y.Z` tag; `.github/workflows/release.yml` builds, signs, and publishes it via GoReleaser, then smoke-tests the published archives on real macOS (Intel + Apple Silicon) and Linux (amd64 natively, arm64 under QEMU) runners. The signing step requires a `SKILLBROWSE_SIGNING_KEY` repository secret (hex-encoded Ed25519 private key; the matching public key is embedded in [`internal/update/verify.go`](internal/update/verify.go)) — see that file's comment for key-rotation instructions. If a release includes a change to the source registry or discovery behavior, call that out explicitly in the release notes by hand (`gh release edit`) — the auto-generated changelog only lists commit messages.
